...
import game_world
from ball import Ball # ball이란 모듈로 부터 Ball이란 클래스를 임포트 해온다
# state event check
...
class Idle:
    ...
    @staticmethod
    def exit(boy, e):
        if space_down(e): # 이벤트가 스페이스 다운이면
            boy.fire_ball()
...
class Boy:
    ...
    def fire_ball(self):
        ball = Ball(self.x, self.y, self.face_dir * 8) # 소년위치, 속도
        game_world.add_object(ball,1) # 1번 레이어 == 소년 위치
